...
Nixを利用してemacsclientを設定することで簡単に始めることができることが分かったのでまとめる。
...
以降、基本的にconfiguration.nixを編集することになる。
...
この設定でOS起動時に同時にemacsclientを起動することができる。

さらに =defaultEditor= を設定することで =EDITOR= をemacscilentを利用するように設定できるため、 =EDITOR= を利用するソフトウェアとも透過的にemacsclientを利用することができる。

環境変数に設定されるだけのため、 =EDITOR= を =e= として起動できるようにしておくと =e something.txt= でファイルを開くことができるようになる。
...
:END:
emacsclientはOS起動時に起動しているため =emacsclient -c= で新規フレームを開くことができる。
=&= を付けてブロックしないようにすると良い。
...
*** ファイルを開く
=e= を =$EDITIOR= に設定しているため、 =e something.txt= でファイルを開くことができる。
開いたシェルに制御を返すには =C-x #= で編集終了を伝えることができる。
